...
class UserModel extends Model {
    ...
    public function obtenerCantPag($busqueda){
        ...
        $consulta="SELECT count(*) total from formulario_recepcion_informacion where ((''+id) like '%$busqueda%' or fecha_declaracion  like '%$busqueda%' or hecho  like '%$busqueda%') and (usuario='$user' or 'admin'='$tipo_user')";
    ...
    public function cargarFormulariosPagina($busqueda,$inicio, $num_formularios_pagina){
        $user=$_SESSION['username_obs'];
        $tipo_user=$this->obtenerInfoUsuario($user)['cargo'];
        $consulta="SELECT id, fecha_declaracion, zona_declaracion, hecho  from formulario_recepcion_informacion where ((''+id) like '%$busqueda%' or fecha_declaracion  like '%$busqueda%' or hecho  like '%$busqueda%') and (usuario='$user' or 'admin'='$tipo_user') limit $inicio,$num_formularios_pagina"; 
        $array=$this->obtenerArrayConsultaUTF8($consulta); 
        return $array;
    }
